...
% ==================== PAGE DE GARDE ====================
\titre{Développement d'un environnement de simulation pour drone de surface aquatique (USV)}
...
\begin{document}
	...
	La pollution marine motive le développement de drones de surface autonomes (USV) capables de collecter les macro-déchets. Ce rapport présente un stage assistant ingénieur consacré à l'implementation de l'environnement de simulation VRX pour un USV. La mission consiste à valider le modèle dynamique de Fossen à 3 degrés de liberté utilisé par la simulation : un protocole de huit expériences excitatrices a été conçu, une chaîne d'acquisition multi-capteurs (pose, IMU, propulseurs) a été développée, et les accélérations prédites par le modèle ont été comparées aux accélérations mesurées, en intégrant les forces de vent et de houle. Les écarts analysés permettent de calibrer les paramètres hydrodynamiques et préparent le développement des lois de guidage point à point nécessaires à la collecte de déchets.
	...
	\begin{description}[font=\normalfont\bfseries]
		...
		\item[LOS] Line-Of-Sight (méthode de guidage)
		\item[RMSE] Root Mean Square Error (erreur quadratique moyenne)
		\item[NED] North-East-Down (repère de navigation)
	...
	Le sujet, intitulé \og Développement d'un environnement de simulation pour drone de surface aquatique (USV)\fg, s'articule autour de quatre axes :
	\begin{enumerate}
		\item \textbf{Configuration de l'écosystème} : mise en place sous Linux de ROS~2, Gazebo et RViz2 ;
		\item \textbf{Intégration et instrumentation} : configuration et calibration virtuelle de la suite sensorielle (caméra RGB, GPS, IMU, LiDAR 3D) du robot ;
		\item \textbf{Traitement de données} : développement de nœuds ROS~2 d'acquisition et de visualisation;
	...
	\end{enumerate}
	Ma contribution s'est concentrée sur les axes~3 et~4, avec pour livrables trois outils logiciels (\texttt{guide.py}, \texttt{logger.py}, \texttt{comp.py}) et une analyse de validation du modèle. Le calendrier du stage a suivi approximativement la répartition du diagramme de Gantt~\ref{fig:gantt_stage} : une phase de prise en main de l'écosystème, une phase de conception et de développement logiciel, puis une phase d'expérimentation et d'analyse.
...
\end{figure}
	...
	\subsection{État de l'art et bibliographie}
	...
	Dans notre projet, RViz2 est principalement utilisé pour observer l'état du WAM-V et vérifier le bon fonctionnement de la chaîne de communication pendant les expérimentations.
	...
	Les échanges de données sont réalisés à travers des topics. Un nœud peut publier des messages à l'aide d'un publisher, tandis qu'un autre nœud peut s'abonner au topic correspondant afin de recevoir ces données. Cette architecture permet de séparer les différentes fonctions du système tout en assurant leur communication.
...
La dynamique des véhicules de surface est classiquement décrite à l'aide du formalisme de Fossen, largement utilisé pour la modélisation et l'étude des systèmes marins \cite{fossen}. Cette approche permet de représenter les différents mouvements du véhicule ainsi que les forces et moments qui s'exercent sur celui-ci. Elle constitue également une base de référence pour la modélisation des voiliers et autres véhicules marins autonomes \cite{lebars,saoud}.

Dans le cadre de ce travail, le drone catamaran est représenté par un modèle à 3 degrés de liberté (\emph{Degrees of Freedom}, DOF), correspondant aux mouvements de translation longitudinale (\emph{surge}), de translation latérale (\emph{sway}) et de rotation autour de l'axe vertical (\emph{yaw}). Cette représentation est adaptée à un véhicule de surface dont les mouvements sont principalement considérés dans le plan horizontal. La formulation en représentation d'état permet alors de décrire l'évolution temporelle de la position, de l'orientation et des vitesses du véhicule \cite{jaulin_etat}.
...
Le modèle retenu prend ainsi en compte les principaux effets dynamiques nécessaires à la simulation du drone, notamment les termes liés à la masse et à l'inertie, les forces et moments hydrodynamiques ainsi que les actions générées par les propulseurs. Cette modélisation constitue la base du modèle dynamique utilisé dans l'environnement de simulations.
	...
	\section{Mise en place de l'environnement de simulation}
	
	\subsection{Chaîne logicielle}
	L'environnement repose sur Ubuntu~22.04, ROS~2 (Jazzy), Gazebo (Harmonic) et le monde \texttt{sydney\_regatta} de VRX, qui fournit le modèle WAM-V équipé de deux propulseurs différentiels, d'une IMU, d'une caméra RGB et d'un LiDAR 3D. RViz2 assure la visualisation des flux capteurs. Ce choix de version (Jazzy + Harmonic) correspond à la configuration officiellement supportée par le dépôt VRX au moment du stage ; il a été privilégié à des versions plus récentes de ROS~2 afin de limiter les risques d'incompatibilité avec les plugins Gazebo fournis par le projet.
	
	\subsection{Architecture de communication}
	Deux middlewares coexistent : le transport Gazebo (topics de simulation : pose, IMU, commandes de propulseurs) et ROS~2 (topic d'étiquetage \verb|/usv/current_segment|). Le tableau~\ref{tab:topics} résume les flux utilisés. Cette coexistence de deux bus de communication est une contrainte structurante du projet VRX : le moteur physique (Gazebo Transport) et la couche applicative (ROS~2, via \texttt{ros\_gz\_bridge}) ne partagent pas nativement le même espace de nommage, ce qui a nécessité un pont explicite pour certains topics et une vigilance particulière sur la synchronisation temporelle des messages lors de l'enregistrement.
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			\midrule
			\verb|/world/sydney_regatta/pose/info| & Gazebo & pose mondiale du WAM-V \\
	...
	L'installation a nécessité la résolution d'incompatibilités de dépendances (versions de paquets ROS/Gazebo, pilotes graphiques). Le parsing des messages Gazebo au format texte (protobuf) a imposé un analyseur robuste par expressions régulières. Enfin, la calibration extrinsèque caméra/LiDAR reste un point de vigilance pour la suite du projet.
	...
	Ce protocole est volontairement riche : les lignes droites excitent le surge, les virages excitent le lacet et le couplage coriolis et la roue libre révèle l'amortissement hydrodynamique.
	...
	\subsection{Justification du choix des consignes}
	...
	\subsection{Séquencement et sécurité du scénario}
	Le séquenceur \texttt{guide.py} publie les consignes à fréquence fixe (20~Hz) et journalise l'étiquette de l'expérience en cours sur le topic \verb|/usv/current_segment|, ce qui permet de segmenter automatiquement l'enregistrement produit par le script de journalisation.
	
	\section{Chaîne d'acquisition des données}
	
	\subsection{Grandeurs enregistrées}
	L'enregistreur fusionne quatre flux asynchrones dans un enregistrement : pose (position, quaternion, cap), vitesses et accélérations corps estimées par différenciation filtrée (Annexe ~\ref{app:mesures}), mesures IMU brutes, commandes propulseurs et étiquette d'expérience. Le repère local est verrouillé sur la première pose reçue, ce qui fournit directement une trajectoire centrée en $(0,0)$.
	
	\subsection{Estimation des vitesses corps}
	Les vitesses monde sont obtenues par différenciation de la pose, puis projetées dans le repère corps à l'aide du quaternion d'attitude ; les vitesses angulaires $(p,q,r)$ sont déduites de la variation de quaternion (Annexe ~\ref{app:mesures}). Un filtre passe-bas du premier ordre ($\alpha_v = 0{,}35$) limite le bruit de différenciation, et un second étage ($\alpha_a = 0{,}25$) produit $\dot u, \dot v, \dot r$.
	...
	\subsection{Contexte historique et motivation}
	
	Le modèle de Fossen fournit un cadre mathématique permettant d'exprimer les équations du mouvement
	non linéaires d'un véhicule marin sous une forme matrice--vecteur compacte. Cette formulation,
	introduite pour la première fois dans sa thèse de doctorat de 1991, offre une
	alternative plus efficace aux modèles hydrodynamiques classiques, lesquels étaient auparavant exprimés
	sous forme de composantes scalaires. Ces modèles antérieurs comportaient souvent des centaines de
	termes individuels, ce qui rendait difficile l'exploitation de propriétés intrinsèques du système
	telles que la passivité, la symétrie et la conservation de l'énergie. L'approche de Fossen est devenue un modèle fondamental en hydrodynamique marine et en
	...
	guidage, de navigation et de contrôle. Elle s'applique à un large éventail de structures
	marines : navires, structures flottantes offshore, sous-marins, véhicules sous-marins autonomes
	et drones de surface autonomes.
	...
	L'équation cinématique~\eqref{eq:cinematique} utilise les angles d'Euler pour décrire la transformation du repère corps vers le repère NED. Les rotations
	...
	une simple rotation planaire d'angle $\psi$ entre repère corps et repère NED, ce qui justifie a
	...
	d'inertie) provient directement de la description URDF/SDF (ce sont des fichiers permettent de décrire la structure et les propriétés physiques du véhicule, ses liens, articulations et capteurs) du catamaran et du fichier de description de l'environement sydney regatta.
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			$\ell$ (\texttt{lever\_arm}) & bras de levier des propulseurs & [2.03] m \\
			\bottomrule
		\end{tabular}
	\end{table}
...
où $\boldsymbol{D}_{L}$ représente les résistances qui varient de manière approximativement proportionnelle à la vitesse, tandis que $\boldsymbol{D}_{NL}$ représente les résistances qui augmentent plus fortement lorsque la vitesse augmente.
	...
	\subsection{Équations du mouvement relatif et forces environnementales}
	...
	Pour un courant océanique irrotationnel, le champ de vitesse est sans rotationnel, ce qui conduit à une
	description en écoulement potentiel : la vitesse du courant $\boldsymbol{\nu}_c$ reste spatialement
	uniforme et constante (ou quasi constante) dans le repère NED. La résolution numérique procède alors
	par intégration de l'équation différentielle portant sur la vitesse absolue. Dans le cadre de nos
	campagnes d'expériences, le monde \texttt{sydney\_regatta} n'imposait pas de courant marin actif ; le terme
...
\begin{itemize}
	\item le véhicule reste à la surface de l'eau et sa position verticale est supposée constante ;
	\item les mouvements de \emph{heave} (translation verticale) sont négligés ;
	\item les mouvements de \emph{roll} (roulis) et de \emph{pitch} (tangage) sont supposés faibles devant les mouvements horizontaux et sont donc négligés ;
	\item les variations de l'assiette du véhicule restent suffisamment faibles pour considérer que sa dynamique est principalement décrite dans le plan horizontal ;
	\item le véhicule est supposé présenter une symétrie approximative bâbord--tribord, ce qui permet de négliger certains termes de couplage ;
	\item les effets de flottabilité et de gravité associés aux mouvements verticaux et angulaires sont donc négligés dans le modèle retenu.
\end{itemize}
...
Les matrices du modèle 3-DOF, telles qu'implémentées dans \texttt{comp1.py}, prennent la forme suivante :

\paragraph{Matrice d'inertie $\boldsymbol{M}$.}
Elle regroupe la masse du véhicule et les effets de masse ajoutée de l'eau :
\begin{equation}
...
\paragraph{Matrice de Coriolis--centripète $\boldsymbol{C}(\boldsymbol{\nu})$.}
Elle représente les effets liés au couplage entre les différents mouvements du véhicule :
\begin{equation}
	\boldsymbol{C}(\boldsymbol{\nu}) = \begin{bmatrix}
...
\paragraph{Matrice d'amortissement $\boldsymbol{D}(\boldsymbol{\nu})$.}
Elle représente les forces de résistance exercées par l'eau sur le véhicule. Les dérivées hydrodynamiques étant négatives, elle est définie par :
\begin{equation}
	\boldsymbol{D}(\boldsymbol{\nu}) =
	-\begin{bmatrix}
		X_u + X_{uu}|u| & 0 & 0 \\
...
\end{equation}
	...
	\subsection{Torseur de propulsion par poussée différentielle}
	
	Les deux propulseurs fixes du WAM-V, alignés avec l'axe $x$ du repère corps et séparés par un bras de
	...
	\subsection{Méthode}
	Pour chaque instant de l'enregistrement, le torseur total $\tau = \tau_{prop} + \tau_{vent} + \tau_{vague}$ est reconstruit, puis calcule l'accélération $\dot\nu = M^{-1}(\tau - C\nu - D\nu)$. Trois courbes sont superposées par axe (surge, sway, yaw) : mesure IMU, dérivée cinématique et résultat de Fossen (figure~\ref{fig:comp}).
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{fossen_comparison.png} % TODO : votre PNG généré
		\caption{Comparaison des accélérations mesurées et prédites par le modèle de Fossen (vent + houle). Les traits verticaux séparent les expériences.}
		\label{fig:comp}
	\end{figure}
...
La lecture visuelle des courbes de la figure~\ref{fig:comp} est complétée par
quatre indicateurs quantitatifs, évalués sur $N$ échantillons entre un signal
mesuré $y^{\mathrm{mes}}$ (accélération, vitesse ou position selon le cas) et
le signal prédit $y^{\mathrm{exp}}$, avec l'erreur $e_i = y^{\mathrm{mes}}_i -
y^{\mathrm{exp}}_i$ :
...
Ces indicateurs sont calculés séparément par axe (surge, sway, yaw).
	
	\subsection{Résultats}
	
	\subsubsection{Expérience n°1}
	\label{sec:exp_1}
	
	\paragraph{Paramètres de l'éprérience.}
	Le jeu de paramètres initial (tableau~\ref{tab:params_exp_1}) reprend des valeurs
	hydrodynamiques raisonnables pour le WAM-V, avec $k = 1$ et $\ell = 1$~m : les consignes de
	propulsion y sont donc interprétées directement comme des newtons et des newtons-mètres.
	
	\begin{table}[H]
		\centering
		\caption{Expérience 1 -- jeu de paramètres initial}
		...
		\end{tabular}
	\end{table}
	\footnotetext{À vérifier : dans ce run, la décomposition journalisée du torseur
		(\texttt{fossen\_errors.csv}) indique des forces de houle nulles ; l'excitation
		environnementale effective se réduit donc au vent moyen.}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{comparison_essai1.png}
		\caption{Expérience 1 -- accélérations mesurées (IMU, cinématique) et prédites par le modèle.
			Les traits verticaux délimitent les expériences.}
		\label{fig:exp_1}
	\end{figure}
	
	\paragraph{Indicateurs globaux.}
	Le tableau~\ref{tab:stats_exp_1} résume les statistiques de l'erreur de prédiction sur les
	$N = 8497$ échantillons.
	
	\begin{table}[H]
		\centering
		\caption{Epérience n°1}
	...
	\paragraph{Observations et interprétation}
	L'analyse conjointe des métriques d'erreur et des séries temporelles montre que le comportement du VRX valide la structure algébrique du modèle de Fossen en 3~DOF, tout en révélant des écarts paramétriques et dynamiques ciblés. En surge ($a_x$ : biais $+0{,}775$~m/s², RMSE $1{,}984$~m/s², MAE $1{,}228$~m/s², écart-type $1{,}826$~m/s²), l'erreur se concentre au début des $8$~segments lors des échelons de poussée : le modèle théorique réagit instantanément ($3{,}6$~m/s² au segment~1 ; $4{,}5$~m/s² au segment~7), alors que la réponse mesurée par l'IMU du VRX présente un retard de $1$ à $2$~s lié à la dynamique propre des propulseurs dans Gazebo. En régime permanent, l'accord est en revanche excellent ($u \approx 2{,}2$~m/s calculé contre $2{,}4$--$2{,}6$~m/s mesuré), confirmant que l'amortissement longitudinal ($X_u, X_{uu}$) décrit fidèlement la réalité du VRX. En sway ($a_y$ : biais $+0{,}004$~m/s², RMSE $0{,}602$~m/s², MAE $0{,}490$~m/s², écart-type $0{,}602$~m/s²), le biais global est nul mais le modèle surestime l'accélération latérale ($\pm 0{,}5$ à $1{,}0$~m/s²) durant les virages (segments~3 à 6) ; cet écart traduit un effet de la réduction 6~DOF$\rightarrow$3~DOF, où l'énergie du terme de Coriolis $280\,u\,r$ est intégralement projetée sur le plan au lieu de se dissiper en roulis et pilonnement comme le fait le VRX dans le simulateur. Enfin, en yaw ($\dot r$ : biais $+0{,}184$~rad/s², RMSE $0{,}335$~rad/s², MAE $0{,}265$~rad/s², écart-type $0{,}280$~rad/s²), le VRX suit la polarité du modèle (inversion de signe fidèlement reproduite entre les segments~5 et 6), mais le modèle surestime nettement la réactivité en rotation avec un taux de lacet théorique ($0{,}75$~rad/s au segment~4) environ $4$~fois supérieur au $r \approx 0{,}2$~rad/s mesuré, ce me prouve que l'amortissement hydrodynamique réel de la coque ($N_r, N_{rr}$) est sous-évalué dans ce jeu de paramètres initial.
	
	\paragraph{Conclusion sur la validité du modèle}
	
	Le comportement du VRX valide la structure théorique du modèle de Fossen en 3~DOF : les équations décrivent correctement les vitesses d'équilibre en ligne droite, la nature des couplages de Coriolis ainsi que la directionnalité et la polarité des couples de rotation. Néanmoins, sa validité quantitative reste partielle sous sa forme idéale actuelle ($\text{RMSE} = 1{,}984$~m/s² en surge et $0{,}335$~rad/s² en yaw). Ces divergences ne remettent pas en cause la formulation mathématique de base, mais soulignent ses limites d'application face à trois facteurs critiques : (i)~les simplifications inhérentes à la réduction 3~DOF (omission des degrés de liberté hors-plan comme le roulis ou le pilonnement et négligence des couplages associés) et (ii)~le caractère stochastique et aléatoire de l'environnement physique sous VRX/Gazebo (bruits de mesure capteurs, variations aléatoires du vent et de la houle).
	
	\subsubsection{Expérience n°2}
	\label{sec:exp_2}
	
	\paragraph{Paramètres de l'éxpérience.}
	L'objectif est double : tester l'influence
	de l'inertie sur le pic d'accélération initial en surge, et isoler la structure hydrodynamique du
	modèle en supprimant les forçages environnementaux.
	\begin{table}[H]
		\centering
		\caption{Expérience 2}
		...
		\begin{tabular}{ll}
			...
			$N_r = -100$, $N_{rr} = -300$ \\
			Propulsion & $k = 1,0$ ; $\ell = 1,0$~m \\
			Vent / houle & \textbf{désactivés} ($V_w = 0$, amplitudes de houle nulles) \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{table}[H]
		\centering
		\caption{Expérience 2 -- statistiques de l'écart modèle/mesures}
	...
	\begin{figure}[H]
		...
		\caption{Expérience 2 -- accélérations mesurées (IMU, cinématique) et sorties du modèle de Fossen
			(vent et houle désactivés).}
		\label{fig:exp_2}
	\end{figure}
	
	\paragraph{Observations et interprétation.} 
	L'isolation du système par la désactivation complète du vent et de la houle permet de révéler la structure hydrodynamique pure du modèle de Fossen. Sur l'axe sway ($a_y$ : biais $0{,}106$~m/s², RMSE $0{,}253$~m/s², MAE $0{,}193$~m/s², MaxAbsError $0{,}780$~m/s² ; $\dot v$ : RMSE $0{,}243$~m/s²), l'examen visuel de la figure montre un accord remarquable : la courbe du modèle (pointillés bleus) épouse presque parfaitement le signal IMU (rouge) sur les régimes permanents en virage, restituant les trois niveaux d'accélération latérale ($0{,}4$, $0{,}8$ et $1{,}1$~m/s² aux segments~3 à 5) ainsi que l'inversion de signe à $-0{,}8$~m/s² (segment~6). La dérivée cinématique $\dot v$ (gris) restant proche de zéro, l'IMU mesure en réalité l'accélération composée $\dot v + r\,u$, ce qui confirme que le terme de Coriolis $(m-X_{\dot u})\,u\,r$ décrit exactement la dynamique latérale du VRX en l'absence de bruits environnementaux. Sur l'axe surge ($a_x$ : biais $0{,}930$~m/s², RMSE $2{,}028$~m/s², MAE $1{,}035$~m/s², MaxAbsError $5{,}923$~m/s²). L'erreur prédominante provient des fronts d'échelon lors des changements de consigne, où le modèle réagit instantanément là où le VRX présente une transition dynamique amortie. Enfin, sur l'axe yaw ($\dot r$ : biais $0{,}112$~rad/s², RMSE $0{,}299$~rad/s², MAE $0{,}223$~rad/s², MaxAbsError $0{,}964$~rad/s²) Fossen prédit une accélération angulaire continue durant toute la durée des virages ($0{,}15$ à $0{,}7$~rad/s²), alors que le VRX stabilisé indique une accélération $\dot r$ nulle après le pic initial, traduisant une sous-estimation de l'amortissement en lacet ($N_r, N_{rr}$).
	
	\paragraph{Conclusion sur la validité du modèle.}
	L'expérience n°2 valide la structure algébrique du modèle de Fossen en 3~DOF, prouvant de manière décisive sur l'axe sway que les couplages non linéaires de Coriolis traduisent la physique du VRX dès lors que les perturbations extérieures sont retirées. Cependant, sa validité quantitative demeure partielle ($\text{RMSE} = 2{,}028$~m/s² en surge et $0{,}299$~rad/s² en yaw). Ces divergences mettent en évidence les frontières d'application de la formulation théorique face à trois limites structurelles : (i)~les simplifications inhérentes à la réduction 3~DOF (omission des couplages verticaux, de roulis et de pilonnement) et (ii)~le caractère stochastique résiduel lié aux bruits de mesure des capteurs et aux imperfections du moteur physique de Gazebo.
	
	\subsubsection{Expérience n°3}
	\label{sec:exp_3}
	...
	\paragraph{Observations et interprétation.}
	L'évaluation du modèle ré-étalonné lors de cette troisième expérience, conduite sous des sollicitations environnementales sévères ($V_w = 15$~m/s, houle $T_p = 2$~s), confirme la capacité de la formulation de Fossen à capturer la dynamique transversale et rotative, tout en faisant émerger une dégradation sur l'axe longitudinal. Sur l'axe sway ($a_y$ : biais $0{,}123$~m/s², RMSE $0{,}375$~m/s², MAE $0{,}310$~m/s², écart-type $0{,}354$~m/s², MaxAbsError $1{,}079$~m/s²), la trajectoire moyenne en pointillés bleus épousant mieux la dynamique de virage malgré l'apparition de fortes oscillations haute fréquence imprimées par la houle à $T_p = 2$~s. Sur l'axe yaw ($\dot r$ : biais $0{,}109$~rad/s², RMSE $0{,}268$~rad/s², MAE $0{,}182$~rad/s², écart-type $0{,}245$~rad/s², MaxAbsError $1{,}094$~rad/s²). En revanche, l'axe surge ($a_x$ : biais $1{,}427$~m/s², RMSE $2{,}251$~m/s², MAE $1{,}586$~m/s², écart-type $1{,}741$~m/s², MaxAbsError $6{,}115$~m/s²), le modèle surestimant systématiquement l'accélération longitudinale en régime permanent et lors des transitoires d'échelon. Ces résultats illustrent le caractère stochastique fort insufflé par le moteur physique de Gazebo sous forte houle et confirment que les perturbations d'environnement ainsi que les couplages 6~DOF$\rightarrow$3~DOF imposent un plancher d'erreur incompressible.
	
	\paragraph{Conclusion sur la validité du modèle.}
	Cette troisième expérience valide la capacité du modèle de Fossen 3~DOF à décrire la dynamique globale du VRX sous environnement perturbé, en particulier pour les couplages de Coriolis en sway et l'amortissement en lacet. Néanmoins, sa validité quantitative globale reste partielle ($\text{RMSE} = 2{,}251$~m/s² en surge, $0{,}375$~m/s² en sway et $0{,}268$~rad/s² en yaw). Ces écarts résiduels démontrent les frontières d'un modèle plan idéal face à trois facteurs limitants majeurs : (i)~les simplifications inhérentes à la réduction 3~DOF qui négligent les transferts d'énergie vers les degrés de liberté hors-plan (roulis, pilonnement, tangage) et (ii)~l'aléa environnemental stochastique propre à Gazebo (vent turbulent, houle stochastique et bruits de mesure capteurs) qui introduit des fluctuations haute fréquence non modélisables par une formulation théorique déterministe.
	...
	\begin{itemize}[label=--]
		...
		\item \textbf{L'expérience~3} a démontré la faisabilité du ré-étalonnage des coefficients d'amortissement en lacet ($N_r, N_{rr}$) sous perturbations sévères, réduisant l'erreur RMSE en yaw de $20\,\%$ et en sway de $34\,\%$, tout en soulignant le compromis d'ajustement inter-axes avec le surge.
	\end{itemize}
	...
	% =====================================================
	\section{Guidage et contrôle du catamaran}
	\label{sec:guidage}
	
	La validation du modèle dynamique (Partie~2) constitue le prérequis indispensable au développement des lois de commande. L'objectif final du projet étant la collecte de macro-déchets, le catamaran doit être capable de naviguer de manière autonome entre des points d'intérêt définis, en compensant les perturbations environnementales. Cette partie détaille l'architecture logicielle, la loi de guidage proportionnelle mise en œuvre et les résultats obtenus en simulation.
	
	\begin{figure}[H]
		\centering
		% TODO : Remplacer par votre capture d'écran RViz ou votre tracé Python de la trajectoire
		\includegraphics[width=0.8\textwidth]{guide.png} 
		\caption{Le but est d'arriver au point cible (déchets).}
		\label{fig:dechet}
	\end{figure}
	
	\subsection{Stratégie de guidage}
	Pour assurer la navigation point à point, nous avons implémenté une loi de guidage proportionnelle, couplée à un contrôleur proportionnel en cap. Le principe est de calculer en temps réel le cap désiré $\psi_c$ pointant vers la cible actuelle $P_c(x_c, y_c)$, puis de générer des commandes de propulsion pour annuler l'erreur de cap $e_\psi$.
	...
	\begin{figure}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tikzpicture}[
				...
				\node[blueblock, below=0.9cm of gps] (imu) {
					\textbf{IMU}\\[2pt]
					\scriptsize Accélérations, Vitesses angulaires
				};
				
				\node[blueblock, fill=bluebg!50, below=1.6cm of imu] (sorties) {
					\textbf{Sorties}\\[3pt]
	...
	Pour évaluer les performances du guidage, un scénario de navigation passant par quatre waypoints formant un rectangle a été exécuté dans le monde \texttt{sydney\_regatta}, avec des conditions de vent modérées ($V_w = 5$~m/s).
	...
	\begin{itemize}[label=--]
		\item \textbf{Convergence vers la ligne de visée :} Après chaque changement de waypoint, le catamaran effectue un virage coordonné pour aligner son cap $\psi$ sur $\psi_d$. La poussée différentielle permet une rotation sur place efficace, validant le couplage lacet-propulsion du modèle.
		...
		\item \textbf{Validation du seuil de basculement :} Le rayon de $4$~m s'est avéré adapté. Le drone ne présente pas d'oscillations excessives au voisinage des waypoints et enchaîne les segments de manière fluide.
	\end{itemize}
	
	\subsection{Limites et perspectives d'amélioration}
	Bien que fonctionnelle pour une navigation basique, la loi de commande proportionnelle présente des limites structurelles mises en évidence lors des tests :
	\begin{enumerate}
		\item \textbf{Erreur statique sous perturbation constante :} En présence d'un courant ou d'un vent latéral constant, un contrôleur purement proportionnel génère une erreur de cap résiduelle non nulle (le drone doit "craboter" légèrement pour compenser, ce qu'un simple P ne permet pas d'atteindre parfaitement sans biais).
		\item \textbf{Réglage du gain $K_p$ :} Un gain trop élevé provoque des dépassements (\emph{overshoot}) et des oscillations en lacet, tandis qu'un gain trop faible rend le système lent à réagir aux changements de cap brusques.
	\end{enumerate}
	
	\textbf{Perspectives :} Pour les futures itérations du projet, l'implémentation d'un correcteur PI (Proportionnel-Intégral-Dérivé) sur l'erreur de cap permettrait d'annuler l'erreur statique grâce au terme intégral. À plus long terme, une loi de guidage LOS avancée, intégrant l'erreur transversale (\emph{cross-track error}) avec une intégration adaptative, offrirait une robustesse accrue pour la collecte précise de déchets en milieu perturbé.
